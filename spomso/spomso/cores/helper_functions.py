...
def generate_grid(size: int | float | tuple | list | np.ndarray,
    ...
    resolution = np.squeeze(resolution)
    ...
    if size.size == 1:
        x = np.linspace(-size / 2, size / 2, co_res_0)
        coor = np.zeros((3, co_res_0))
        coor[0] = x

    if size.size == 2:
        x = np.linspace(-size[0] / 2, size[0] / 2, co_res_0)
        y = np.linspace(-size[1] / 2, size[1] / 2, co_res_1)
    ...
    if size.size == 3:
        x = np.linspace(-size[0] / 2, size[0] / 2, co_res_0)
        y = np.linspace(-size[1] / 2, size[1] / 2, co_res_1)
        z = np.linspace(-size[2] / 2, size[2] / 2, co_res_2)
