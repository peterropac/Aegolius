...
class GenericGeometry(EuclideanTransform, ModifyObject):
    ...
    def __repr__(self):
        ...
        try:
            ...
            for name in param_names:
                if hasattr(self, f"_{name}"):
                    pairs.append(f"{name}={self._short_repr(getattr(self, f'_{name}'))}")
                else:
                    pairs.append(f"{name}=?")
            params = ", ".join(pairs)
        ...
        base = f"{cls.__name__}({params})"
        parts = [base]
        if self.modifications:
            parts.append(f"Modifications: [{' → '.join(self.modifications)}]")
        if self.transformations:
            parts.append(f"Transformations: [{' → '.join(self.transformations)}]")
        return "  ".join(parts)
    ...
    def _short_repr(value, max_len=50):
        ...
        if callable(value):
            return f"<{getattr(value, '__name__', 'callable')}>"
        r = repr(value)
        if len(r) > max_len:
            return r[:max_len - 3] + "..."
        return r
...
class Points(EuclideanTransformPoints):
    ...
    def __repr__(self):
        if self._points.size <= 6:
            parts = [f"Points(points={repr(self._points.tolist())})"]
        else:
            parts = [f"Points(points=ndarray(shape={self._points.shape}, dtype={self._points.dtype}))"]
        if self.transformations:
            parts.append(f"Transformations: [{' → '.join(self.transformations)}]")
        return "  ".join(parts)
...
class VectorField(ModifyVectorObject):
    ...
    def theta(self, p: np.ndarray, *parameters_: tuple) -> np.ndarray:
        ...
        cos_theta = np.clip(vec[2] / safe_mag, -1.0, 1.0)
        return np.where(mag == 0, 0.0, np.arccos(cos_theta))
