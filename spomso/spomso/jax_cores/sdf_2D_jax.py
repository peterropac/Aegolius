...
def sdf_box_2d(co: array_like_type, size: array_like_type) -> jnp.ndarray:
    ...
    d = jnp.subtract(jnp.abs(co[:2, :]).T, v).T
...
def sdf_rounded_box_2d(co: array_like_type, size: array_like_type, rounding: array_like_type) -> jnp.ndarray:
    ...
    v = jnp.asarray(size) / 2
    d = jnp.subtract(jnp.abs(co[:2, :]).T, v).T + r
...
def sdf_triangle_2d(co: array_like_type, p0: array_like_type, p1: array_like_type, p2: array_like_type) -> jnp.ndarray:
    ...
    Args:
        ...
        p1: Vector defining the position of the second vertex (D = 2, 3).
        p2: Vector defining the position of the third vertex (D = 2, 3).
    ...
    v0 = jnp.subtract(co[:2,:].T, p0[:2]).T
    v1 = jnp.subtract(co[:2,:].T, p1[:2]).T
    v2 = jnp.subtract(co[:2,:].T, p2[:2]).T
